...
class CommandUploadMixin(CommandManageMixin):
    ...
    def _build_category_stats(self, to_upload: list, to_download: list) -> list:
        ...
        if upload_stats:
            lines.extend(
                self._build_category_section("📤 待上传文件分类:", upload_stats)
            )
        download_stats = self._count_by_category(to_download)
        if download_stats:
            lines.extend(
                self._build_category_section("📥 待下载文件分类:", download_stats)
            )
        return lines
    ...
    def _build_file_examples(self, to_upload: list, to_download: list) -> list:
        lines = []
        lines.extend(
            self._build_file_example_section("📤 待上传文件示例（前5个）:", to_upload)
        )
        lines.extend(
            self._build_file_example_section("📥 待下载文件示例（前5个）:", to_download)
        )
        return lines
