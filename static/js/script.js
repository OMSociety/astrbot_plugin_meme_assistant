document.addEventListener("DOMContentLoaded", () => {
  ...
  async function identifyAll() {
    const btn = identifyAllBtn;
    try {
      setButtonBusy(btn, "提交中...");
      const data = await requestJson(
  ...
  async function reidentifyAll() {
    const btn = reidentifyAllBtn;
    const confirmed = await showConfirm({
      title: "全量重识别",
      description: "这将清除所有已有描述，重新用 LLM 识别全部表情包。该操作不可撤销，确定要执行吗？",
      confirmLabel: "确认重识别",
      confirmClassName: "danger",
    });
    if (!confirmed) return;
  ...
  identifyAllBtn.addEventListener("click", identifyAll);
  reidentifyAllBtn.addEventListener("click", reidentifyAll);
